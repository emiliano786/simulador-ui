    ...
    <template #main-content>
      <div class="max-w-7xl mx-auto space-y-6">
        ...
        >
          ...
          <div class="p-8">
            <div
              class="mb-8 flex items-center justify-between p-4 bg-gray-50/50 dark:bg-slate-900/50 rounded-xl border border-gray-100 dark:border-slate-800"
            >
              <div class="flex items-center gap-4">
                <div
                  class="relative inline-block w-10 h-5 transition duration-200 ease-in"
                >
                  <input
                    type="checkbox"
            ...
            <div
              class="grid grid-cols-1 md:grid-cols-4 gap-x-8 gap-y-6 transition-all duration-300"
              :class="{
                'opacity-30 pointer-events-none grayscale': isSyntheticMode,
              }"
            >
              <e-input
            ...
            <div class="flex justify-end mt-12">
              <button
                ...
                class="px-12 py-3 rounded-xl font-bold text-sm transition-all flex items-center gap-3 bg-[#8b5cf6] text-white hover:bg-[#7c3aed] shadow-lg shadow-purple-500/20 disabled:bg-gray-300 dark:disabled:bg-slate-800 disabled:shadow-none disabled:cursor-not-allowed"
              >
                <span
                  v-if="isLoading"
                ...
                <span>{{
                  isSyntheticMode
                    ? `Ejecutar Ráfaga (${batchSize} msgs)`
        ...
        >
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              ...
              <div
                class="bg-gray-50 dark:bg-black p-4 rounded-xl border border-gray-100 dark:border-slate-800 font-mono text-[10px] text-gray-500 dark:text-gray-400 break-all leading-relaxed max-h-32 overflow-y-auto custom-scrollbar"
              >
...
<script setup>
import { reactive, ref, computed, watch } from "vue"; // <-- ¡Agregamos watch!
...
// --- NUEVO: Observador para recortar el monto a 12 caracteres máximo ---
watch(
  () => form.monto,
  (nuevoValor) => {
    if (nuevoValor !== null && nuevoValor !== undefined && nuevoValor !== "") {
      const valorString = String(nuevoValor);
      if (valorString.length > 12) {
        form.monto = valorString.slice(0, 12);
      }
    }
  },
);
...
const isFormValid = computed(() => {
  ...
  // 3. Validar Monto (no vacío y mayor a 0)
  const montoFloat = parseFloat(form.monto);
  const isValidMonto =
    form.monto !== null &&
    form.monto !== "" &&
    !isNaN(montoFloat) &&
  ...
  // 4. Retornamos true solo si TODO lo obligatorio se cumple.
  return (
    isValidTarjeta &&
    isValidAfiliacion &&
    isValidGiro &&
    isValidPais &&
    isValidMonto && // <-- Agregada validación del monto
    isValidFecha &&
    isValidHora &&
    isValidCanal
  );
