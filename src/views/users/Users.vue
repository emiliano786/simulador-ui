    ...
    >
      <div>
        ...
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
          Administra accesos y permisos del simulador
        </p>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          >
            ...
            >
              <td class="px-6 py-4 font-bold text-cyan-600 dark:text-cyan-400">
                {{ user.user }}
              </td>
              ...
              <td class="px-6 py-4">
                <span
                  class="bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider"
                >
                  {{ user.role }}
                </span>
              </td>
              <td class="px-6 py-4 text-center">
                <span
                  v-if="user.active"
                  class="bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 w-fit mx-auto"
                >
                ...
                <span
                  v-else
                  class="bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 w-fit mx-auto"
                >
      ...
      >
        ...
        >
          ...
          >
            <div class="p-6 overflow-y-auto custom-scrollbar space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    required
                    class="w-full text-xs font-mono px-3 py-2.5 rounded-lg border bg-gray-50 dark:bg-black text-gray-800 dark:text-white outline-none focus:border-cyan-500 border-gray-200 dark:border-slate-700"
                  />
                </div>
                <div>
                  <label
              ...
              <div>
                ...
                <input
                  ...
                  :disabled="isEditing"
                  required
                  class="w-full text-xs font-mono px-3 py-2.5 rounded-lg border bg-gray-50 dark:bg-black text-gray-800 dark:text-white outline-none focus:border-cyan-500 border-gray-200 dark:border-slate-700 disabled:opacity-50"
                />
              </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    required
                    class="w-full text-xs font-mono px-3 py-2.5 rounded-lg border bg-gray-50 dark:bg-black text-gray-800 dark:text-white outline-none focus:border-cyan-500 border-gray-200 dark:border-slate-700 appearance-none"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    :required="!isEditing"
                    :placeholder="
                      isEditing ? 'Vacía para no cambiar' : 'Min. 8 caracteres'
                    "
                    class="w-full text-xs font-mono px-3 py-2.5 rounded-lg border bg-gray-50 dark:bg-black text-gray-800 dark:text-white outline-none focus:border-cyan-500 border-gray-200 dark:border-slate-700"
                  />
                </div>
              ...
              <div class="pt-2">
                ...
                >
                  <input
                    ...
                    class="w-4 h-4 text-cyan-600 bg-gray-100 border-gray-300 rounded focus:ring-cyan-500 dark:bg-gray-700 dark:border-gray-600 accent-cyan-500"
                  />
                  <span
            ...
            <div
              class="px-6 py-4 border-t border-gray-100 dark:border-slate-800 flex justify-end gap-3 bg-gray-50/50 dark:bg-slate-900/30"
            >
              <button
                type="button"
                @click="closeUserModal"
                class="px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
              >
                Cancelar
              </button>
              <button
                ...
                class="px-5 py-2.5 bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-md flex items-center gap-2 disabled:opacity-50"
              >
                <span
                  v-if="isSubmitting"
                  class="animate-spin border-2 border-white/30 border-t-white rounded-full w-3 h-3"
                ></span>
                {{
                  isSubmitting
                    ? "Guardando..."
...
const submitUser = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      await axios.put(
        `http://localhost:8080/api/users/${editingUserId.value}`,
        userForm.value,
        config,
      );
    ...
    showToast(
      err.response?.data?.detail || "Hubo un error al guardar el usuario.",
      "error",
    );
  } finally {
...
const executeDelete = async () => {
  ...
  try {
    const token = localStorage.getItem("iso_token");
    await axios.delete(
      `http://localhost:8080/api/users/${userToDelete.value.id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
