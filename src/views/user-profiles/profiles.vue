<template>
  <content-tpl>
    ...
    <template #main-content>
      <div class="flex flex-col h-fit gap-3">
        ...
        >
          <!-- Slot para acciones -->
          <template v-if="hasAnyProfileActionPermission" #cell-name-accion="{ rowOrig }">
            <td>
              <div class="flex w-full justify-center items-center gap-2">
                <!-- Botón editar -->
                <div
                  v-if="canUpdate('profiles')"
  ...
  <e-dialog
    ...
    :title="$t('profiles.form.confirmation.deleteTitle')"
      ...
      <div class="w-[60vw] max-w-md max-h-[80vh] overflow-y-auto">
        <div class="flex flex-col items-center justify-center py-8 px-6">
          ...
          <h3 class="text-title-2 font-bold text-danger dark:text-danger/90 mb-3 text-center">
            {{ $t('profiles.form.confirmation.deleteInfoTitle') }}
          </h3>

          <!-- Mensaje descriptivo -->
          <p class="text-body-3 text-gray-700 dark:text-gray-300 text-center mb-6 leading-relaxed">
            <span class="block">
              {{ $t('profiles.form.confirmation.deleteInfoMessage') }}
            </span>
            <span
              class="block font-mono font-bold text-danger dark:text-danger-2 text-body-2 px-2 py-1 bg-danger/10 dark:bg-danger/20 rounded-lg border border-danger/20 dark:border-danger/30 shadow-sm"
            >
              {{ profileToDelete?.name }} {{ profileToDelete?.lastname }}
            </span>
            <span class="block">
              {{ $t('profiles.form.confirmation.deleteInfoMessageEnd') }}
            </span>
          </p>
          ...
          >
            <div class="flex items-center gap-2 mb-2">
              ...
              <span class="text-body-3 text-danger dark:text-danger/90">
                {{ $t('profiles.form.general.profileToDelete') }}
              </span>
            </div>
            <ul class="text-caption-2 text-gray-600 dark:text-gray-400 space-y-1 ml-4">
              <li class="flex items-center gap-1">
                ...
                <span
                  >{{ profileToDelete?.name }}<br />
                  {{ profileToDelete?.lastname }}</span
                >
              </li>
              <li class="flex items-center gap-1">
                <e-icon name="account_circle" size="xs" color="text-danger" />
                <span>{{ profileToDelete?.description }}</span>
              </li>
            </ul>
          </div>
          ...
          <div class="flex justify-end gap-3 w-full">
            ...
            <e-btn
              level="1"
              :text="$t('profiles.form.actions.deleteProfile')"
              iconName="delete"
  ...
  <!-- Diálogo de éxito o error tras eliminación -->
  <e-dialog
        ...
        <div class="flex flex-col items-center justify-center py-8 px-6">
          ...
          <p class="text-body-3 text-gray-700 dark:text-gray-300 text-center mb-6 leading-relaxed">
            <template v-if="deleteSuccess">
              {{ t('profiles.form.success.deleteMessage') }}
              <span
            ...
            <template v-else>
              ...
              <span v-if="deleteErrorMsg" class="block mt-2 text-caption-2 text-gray-500">
                {{ t('profiles.delete.error.detail', { msg: deleteErrorMsg }) }}
              </span>
            </template>
          </p>
...
<script setup>
  ...
  const hasAnyProfileActionPermission = computed(() => {
    return canUpdate('profiles') || canDelete('profiles');
  });
  ...
  const columns = computed(() => {
    const baseColumns = [
      ...
      {
        title: t('profiles.form.general.profileName'),
        field: 'name',
        sortable: true,
        required: true
      },
      {
        title: t('profiles.form.general.profileDescription'),
        field: 'description',
        sortable: true,
        required: true
      },
      {
        title: t('profiles.form.general.status'),
        field: 'statusLabel',
        sortable: true,
        required: true
      }
    ...
    if (hasAnyProfileActionPermission.value) {
      baseColumns.push({
        title: t('profiles.form.general.actions'),
        field: 'accion',
        sortable: false,
        required: true
      });
  ...
  const rowsForTable = computed(() =>
    profilesStore.profiles.map((r) => ({
      ...r,
      statusLabel: r.status ? 'Activo' : 'Inactivo'
    }))
  );

  // Total de registros para la paginación
  const totalRecords = computed(() => profilesStore.total);

  // Cargar los perfiles al montar el componente
  onMounted(async () => {
    loading.show({ project: 'services' });
    try {
      await profilesStore.listProfiles({ page: 1, limit: 10 });
    } finally {
      setTimeout(() => {
        loading.hide();
      }, 800);
  ...
  const goToEditProfile = async (profile) => {
    await profilesStore.fetchProfileDetail(profile._id);
    router.push({ name: 'create-profile' });
  };
  ...
  const confirmDeleteProfile = async () => {
    try {
      loading.show({ project: 'services' });
      const resp = await profilesStore.deleteProfile(profileToDelete.value._id);
      loading.hide();
      showDeleteDialog.value = false;
      deleteSuccess.value = resp.success === true;
      profilesStore.listProfiles({ page: 1, limit: 10 });
